...
print(f"Inactive users using filter : \n {inactive_users} ")

#extract inactive users using list comprehension:
inactive_users2= [user for user in users if not user["tweets"]]
print(f"Inactive users using list comprehension : \n {inactive_users2} ")

# extract usernames of inactive users w/ map and filter:
usernames = list(map(lambda user: user["username"].upper(), 
	filter(lambda u: not u['tweets'], users)))
print(f"Usernames of inactive users w/ map and filter: \n {usernames} ")
...
print(f"Usernames of inactive users w/ list comprehension: \n {usernames2} ")
